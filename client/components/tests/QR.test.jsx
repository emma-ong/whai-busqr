...
describe('QR', () => {
  ...
  it('h1 contains - unique', async () => {
    QRCode.toDataURL.mockReturnValue(Promise.resolve(encoded))
    expect.assertions(1)
      act(() => {render(<Router><QR/></Router>)})
      await waitFor(() => QRCode.toDataURL)
      const h1 = screen.getByRole('heading', 'h1')
      await waitFor(() => {expect(h1).toHaveTextContent('unique')})
  ...
  it('selects the right option properly', async () => {
    ...
    act(() => {render(<Router><QR/></Router>)})
      ...
      const option = screen.getByRole('option', {name: 'Default: 0.15m'})
